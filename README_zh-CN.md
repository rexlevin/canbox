...
以下功能计划在未来开发。详细设计文档可以在 `openspec/changes/` 中找到：

### Matrix 账号体系

基于 Matrix 协议的去中心化账号系统，用于跨设备数据同步和未来的社交功能（Chat、Blog）。

**核心能力：**

- Matrix 认证支持多种登录方式（密码、Mozilla SSO 等）
- 跨设备数据同步（APP 列表、设置、偏好）
- 智能冲突解决（APP 版本比较合并、配置最新优先）
- 离线队列支持持续操作

**未来扩展：**

- Matrix Chat 集成
- Blog 通过 ActivityPub 桥接到 Fediverse
...
- **临时目录**：设置应用临时文件目录
- **日志保留天数**：配置日志文件保留天数（1-365 天，默认 30 天）
- **立即清理日志**：手动立即清理旧日志文件
